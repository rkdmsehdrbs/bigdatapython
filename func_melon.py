...
def save_top_100_to_file(title, filename="melon_top100.txt"):
  ...
  # main.py
import func # func.py 파일의 함수들을 사용하기 위해 import
import time

# 각 메뉴 제목 (선택적, 직접 문자열을 넘겨도 됨)
title_m100 = "<멜론 차트 TOP 100곡>"
title_m50 = "<멜론 차트 TOP 50곡>"
title_m10 = "<멜론 차트 TOP 10곡>"
title_random = "<멜론 차트 AI 추천곡>"
title_search = "<가수 이름 검색>"
title_save = "<파일에 저장(멜론 100)>"
...
while True: # 프로그램 반복 실행
  ...
  print("| 5. 가수 이름 검색       |")
  print("| 6. 파일에 저장(멜론 100)|") # 파일 저장 옵션 추가
  print("| 0. 종료                |") # 종료 옵션
  print("==========================")
  ...
  # 입력에 따른 기능 실행
  if n == "1":
    func.show_top_n(title_m100, 100)
  elif n == "2":
    func.show_top_n(title_m50, 50)
  elif n == "3":
    func.show_top_n(title_m10, 10)
  elif n == "4":
    func.recommend_song(title_random)
  elif n == "5":
    s = input("[검색하고 싶은 가수의 이름을 입력하세요.]: ")
    func.search_artist(title_search, s)
  elif n == "6":
    func.save_top_100_to_file(title_save)
  elif n == "0":
    print("[프로그램을 종료합니다.]")
    break # 반복문 탈출
  else:
    print(f"[<{n}>번에 해당하는 서비스가 없어요. 0~6번 중에 선택해 주세요.]")

  # 다음 메뉴 표시 전 약간의 지연 (선택 사항)
  print("\n(잠시 후 메뉴로 돌아갑니다...)")
  time.sleep(2)